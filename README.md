...
где параметр `name` соответствует полю `code_mp` в базе данных. Хост и порт выносятся в конфигурационный файл.
...
Поле `source` имеет вид 
```
rtsp://login:pass@ip:554/cam_path
```

где значения `login`, `pass` и `ip` берутся из таблицы базы данных ( поля `login`, `pass` и `ip_address_out` соответстенно). Шаблон поля `runOnReady` находится в конфигурационном файле и также заполняется с помощью значений из базы данных.
